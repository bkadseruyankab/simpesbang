...
export function DashboardPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['dashboard', tahun, bulan, jenisKendaraan],
    queryFn: async () => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Ringkasan data service kendaraan operasional dinas</p>
        </div>
      ...
      <Card className="border-0 shadow-md">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-semibold flex items-center gap-2">
            <Activity className="h-4 w-4 text-teal-600" /> Progress Perbaikan Kendaraan
          </CardTitle>
          <CardDescription className="text-xs">Kendaraan yang sedang dalam proses perbaikan</CardDescription>
        </CardHeader>
        <CardContent>
          {(stats.progressPerbaikan?.length > 0) ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-xs text-muted-foreground">Tidak ada kendaraan dalam proses perbaikan</p>
            </div>
          )}
        </CardContent>
      </Card>
